...
for (const locale of ['en', 'es']) {
  ...
  const localeArchiveBody = `<main class="tt-blog-static"><header><p>COMPLETE ARCHIVE</p><h1>${escapeHtml(locale === 'en' ? 'Every Tironi Tech article' : 'Todos los artículos de Tironi Tech')}</h1><p>${localeArticles.length} articles available. The 120 priority guides remain highlighted in the main library.</p></header><nav><a href="/${locale}/blog">Blog</a></nav><section>${localeArticles.slice(0, 18).map((article) => `<article><p>${escapeHtml(article.category)} · ${escapeHtml(article.readTime)}</p><h3><a href="${localizedPath(article.slug, locale)}">${escapeHtml(article.title)}</a></h3><p>${escapeHtml(article.description)}</p></article>`).join('')}</section></main>`;
  const archiveDirectory = resolve(root, locale, 'blog', 'archive');
  await mkdir(archiveDirectory, { recursive: true });
  await writeFile(resolve(archiveDirectory, 'index.html'), pageTemplate({ title: `${locale === 'en' ? 'Complete blog archive' : 'Archivo completo del blog'} | Tironi Tech`, description: `${localeArticles.length} Tironi Tech articles.`, path: localeArchivePath, body: localeArchiveBody, schema: { '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'Tironi Tech complete blog archive', url: `${origin}${localeArchivePath}` }, locale, robots: 'noindex, follow' }));
  ...
  for (const article of localeArticles) {
    ...
    const schema = { '@context': 'https://schema.org', '@graph': [{ '@type': 'BlogPosting', headline: article.title, description: article.description, datePublished: article.date, dateModified: article.updated, inLanguage: locale, articleSection: article.category, wordCount, author: { '@type': 'Organization', name: 'Tironi Tech', url: origin }, publisher: { '@type': 'Organization', name: 'Tironi Tech', url: origin }, mainEntityOfPage: `${origin}${path}`, keywords: article.keywords.join(', '), citation: article.sources.map((source) => source.url) }, { '@type': 'FAQPage', mainEntity: article.faqs.map((faq) => ({ '@type': 'Question', name: faq.question, acceptedAnswer: { '@type': 'Answer', text: faq.answer } })) }] };
...
const staticUrls = [
  ...
  { path: '/en/blog', lastmod: '2026-09-22', priority: '0.9', frequency: 'weekly' },
  { path: '/es/blog', lastmod: '2026-09-22', priority: '0.9', frequency: 'weekly' },
