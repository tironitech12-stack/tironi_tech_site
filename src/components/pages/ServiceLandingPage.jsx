...
export default function ServiceLandingPage({ page }) {
  ...
  return <div className="tt2-page tt-service-page"><div className="tt2-page-inner">
    ...
    <main className="tt2-site-main">
      ...
      <section className="tt-service-solution"><div className="tt2-container tt-service-two-col"><div><span className="tt-service-kicker">ESCOPO VERIFICÁVEL</span><h2>O que precisa ficar claro antes de contratar</h2></div><div><p>Uma proposta responsável descreve o processo atual, o resultado esperado, os dados necessários, as integrações, as exceções e quem aprova ações sensíveis. Para este cenário, o ponto de partida é: {page.problem}</p><p>A entrega deve incluir mapa do fluxo, arquitetura e permissões, critérios de aceite, plano de implantação, documentação e indicadores. A solução é avaliada pelo processo que melhora, e não pela quantidade de recursos ou pela promessa genérica de usar IA.</p><ul>{page.outcomes.map((item) => <li key={item}>{item}</li>)}</ul></div></div></section>
